...
var props = { badgeProps: { type: Object }, disabled: { type: Boolean, value: false }, icon: { type: null }, label: { type: String, value: "" }, lazy: { type: Boolean, value: false }, panel: { type: String }, value: { type: null } };
...
// node_modules/tdesign-miniprogram/miniprogram_dist/tab-panel/tab-panel.js
var { prefix: prefix2 } = config_default, name = `${prefix2}-tab-panel`;
var TabPanel = class extends SuperComponent {
  constructor() {
    super(...arguments), this.externalClasses = [`${prefix2}-class`], this.relations = { "../tabs/tabs": { type: "ancestor" } }, this.options = { multipleSlots: true }, this.properties = props_default, this.data = { prefix: prefix2, classPrefix: name, active: false, hide: true, id: "", hasActivated: false }, this.observers = { "label, badgeProps, disabled, icon, panel, value, lazy"() {
      this.update();
    } };
  }
  setId(e) {
  ...
  getComputedName() {
    return null != this.properties.value ? `${this.properties.value}` : `${this.index}`;
  }
  update() {
    var e;
  ...
  render(e, t) {
    this.initialized = this.initialized || e, e && !this.data.hasActivated && this.setData({ hasActivated: true }), this.setData({ active: e, hide: !t.data.animation && !e });
  }
};
