...
// node_modules/tdesign-miniprogram/miniprogram_dist/tabs/index.js
var index_exports = {};
...
var props = { animation: { type: Object }, bottomLineMode: { type: String, value: "fixed" }, showBottomLine: { type: Boolean, value: true }, spaceEvenly: { type: Boolean, value: true }, split: { type: Boolean, value: true }, sticky: { type: Boolean, value: false }, stickyProps: { type: Object }, swipeable: { type: Boolean, value: true }, theme: { type: String, value: "line" }, value: { type: null, value: null }, defaultValue: { type: null } };
...
var MinDistance = 10, getDirection = (t, s) => t > s && t > 10 ? "horizontal" : s > t && s > 10 ? "vertical" : "";
var touch_default = Behavior({ methods: { resetTouchStatus() {
...
var { prefix: prefix2 } = config_default, name = `${prefix2}-tabs`, getUniqueID = uniqueFactory("tabs");
var Tabs = class extends SuperComponent {
  constructor() {
    super(...arguments), this.options = { pureDataPattern: /^currentLabels$/ }, this.behaviors = [touch_default], this.externalClasses = [`${prefix2}-class`, `${prefix2}-class-item`, `${prefix2}-class-active`, `${prefix2}-class-track`, `${prefix2}-class-content`], this.relations = { "../tab-panel/tab-panel": { type: "descendant", linked(t) {
    ...
    } } }, this.properties = props_default, this.controlledProps = [{ key: "value", event: "change" }], this.observers = { value(t) {
      t !== this.getCurrentName() && this.setCurrentIndexByName(t);
    } }, this.data = { prefix: prefix2, classPrefix: name, tabs: [], currentLabels: [], currentIndex: -1, trackOption: { lineWidth: 0, distance: 0, isInit: true }, offset: 0, scrollLeft: 0, tabID: "", placement: "top" }, this.lifetimes = { created() {
      ...
      const { bottomLineMode: t } = this.properties, e = { fixed: `.${prefix2}-tabs__track`, auto: `.${prefix2}-tabs__item--active .${prefix2}-tabs__item-inner`, full: `.${prefix2}-tabs__item--active` };
      return new Promise((i, s) => {
        this.trackWidth ? i(this.trackWidth) : getRect(this, e[t] || e.fixed).then((t2) => {
          t2 && i(t2.width);
        }).catch(s);
      });
    }, setTrack() {
      return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!(e <= -1)) try {
          const t2 = yield getRect(this, `.${prefix2}-tabs__item`, true), i = t2[e];
          if (!i) return;
          let s = 0, r = 0, a = 0;
          if (t2.forEach((t3) => {
          ...
          } else this._hasObserved || (this._hasObserved = true, getObserver(this, `.${name}`).then(() => this.setTrack()));
