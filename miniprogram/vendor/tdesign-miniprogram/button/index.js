...
var { prefix: prefix2 } = config_default, name = `${prefix2}-button`;
var Button = class extends SuperComponent {
  constructor() {
    super(...arguments), this.externalClasses = [`${prefix2}-class`, `${prefix2}-class-icon`, `${prefix2}-class-loading`], this.behaviors = canIUseFormFieldButton() ? ["wx://form-field-button"] : [], this.properties = props_default, this.options = { multipleSlots: true }, this.data = { prefix: prefix2, className: "", classPrefix: name }, this.observers = { "theme, size, plain, block, shape, disabled, loading, variant"() {
      this.setClass();
    }, icon(t) {
      this.setData({ _icon: calcIcon(t, "") });
    } }, this.lifetimes = { attached() {
      this.setClass();
    } }, this.methods = { setClass() {
      const t = [name, `${prefix2}-class`, `${name}--${this.data.variant || "base"}`, `${name}--${this.data.theme || "default"}`, `${name}--${this.data.shape || "rectangle"}`, `${name}--size-${this.data.size || "medium"}`];
