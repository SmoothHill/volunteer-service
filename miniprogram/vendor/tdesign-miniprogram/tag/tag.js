...
var props = { closable: { type: null, value: false }, disabled: { type: Boolean, value: false }, icon: { type: null }, maxWidth: { type: null }, shape: { type: String, value: "square" }, size: { type: String, value: "medium" }, theme: { type: String, value: "default" }, variant: { type: String, value: "dark" } };
...
// node_modules/tdesign-miniprogram/miniprogram_dist/tag/tag.js
var { prefix: prefix2 } = config_default, name = `${prefix2}-tag`;
var Tag = class extends SuperComponent {
  constructor() {
    super(...arguments), this.data = { prefix: prefix2, classPrefix: name, className: "", tagStyle: "" }, this.properties = props_default, this.externalClasses = [`${prefix2}-class`], this.options = { multipleSlots: true }, this.lifetimes = { attached() {
      ...
      this.setData({ _icon: calcIcon(s) });
    }, closable(s) {
      this.setData({ _closable: calcIcon(s, "close") });
    } }, this.methods = { setClass() {
      const { prefix: s, classPrefix: t } = this.data, { size: e, shape: a, theme: i, variant: o, closable: l, disabled: r } = this.properties, c = classNames([t, `${t}--${i || "default"}`, `${t}--${o}`, l ? `${t}--closable ${s}-is-closable` : "", r ? `${t}--disabled ${s}-is-disabled` : "", `${t}--${e}`, `${t}--${a}`]);
      this.setData({ className: c });
    }, setTagStyle() {
      const { maxWidth: s } = this.properties;
      if (!s) return "";
      const t = isNumeric(s) ? `${s}px` : s;
